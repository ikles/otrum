    ...
    protected function _getData($count, $startIndex) {
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            $r['image'] = NextendImageFallBack::fallback($url, array(
                !empty($result[$i]['file_path']) ? 'images/com_hikashop/upload/' . $result[$i]['file_path'] : '',
            ), array(
                @$r['description']
            ));

            if (!empty($result[$i]['file_path'])) {
                $r['thumbnail'] = str_replace('images/com_hikashop/upload/', 'images/com_hikashop/upload/thumbnails/100x100/', $r['image']);
            } else {
                $r['thumbnail'] = $r['image'];
            }
