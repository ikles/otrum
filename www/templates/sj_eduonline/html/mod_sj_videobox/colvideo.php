...
<?php }?>
	...
	if( $key == 0 ){?>
		<iframe id="sj_videobox_show<?php echo $module->id;?>" width="100%" height="400px" src="<?php echo $ur.$url.$auto;?>" frameborder="0" allowfullscreen></iframe>
	<?php }}?>
	</div></div>
        <div class="col-md-6">
	<div id="myCarousel<?php echo $suffix;?>" class="sj-video-list row <?php if( $params->get('theme') == 1 ){echo " style2 carousel slide";} ?>">
		...
		<?php $i=0; foreach ($items as $value){ $i++; $url = trim($value['url']);?>
			...
			<div class="<?php if( $i == 1 ){echo ' active ';}?>  col-md-<?php echo 12/$item_page;?>">
			<?php }?>
				<div data-target="#sj_videobox_show<?php echo $module->id;?>" data-url="<?php echo $ur.$url.$auto; ?>" class="sj-video-item col-md-<?php echo 12/$item_page;?>  <?php if( $i%2 == 0 ){}if( $i%$item_page == 1 && $params->get('theme') == 1 ){echo ' first';}if( $i == 1 ){echo ' selected';}?>">
                    ...
                    <?php } else { ?>
                    <div class="sj-video-image">
                        <img src="http://img.youtube.com/vi/<?php echo $url; ?>/maxresdefault.jpg" title="">
                    </div>    
					<?php }?>
