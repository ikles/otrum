...
<?php }?>
	...
	if( $key == 0 ){?>
		<iframe id="sj_videobox_show<?php echo $module->id;?>" width="100%" height="210" src="<?php echo $ur.$url.$auto;?>" frameborder="0" allowfullscreen></iframe>
	<?php }}?>
	</div>
	<div id="myCarousel<?php echo $suffix;?>" class="sj-video-list  <?php if( $params->get('theme') == 1 ){echo " style2 carousel slide";}?>">
		...
		<?php $i=0; foreach ($items as $value){ $i++; $url = trim($value['url']);?>
			...
			<?php }?>
				<div data-target="#sj_videobox_show<?php echo $module->id;?>" data-url="<?php echo $ur.$url.$auto; ?>" class="sj-video-list-item <?php if( $i%2 == 0 ){}if( $i%$item_page == 1 && $params->get('theme') == 1 ){echo ' first';}if( $i == 1 ){echo ' selected';}?>">
