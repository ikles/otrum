			...
			<?php foreach ($items as $key=>$item):	?>
				<li class="<?php echo ($key%2) ? "odd" : "even"; if(count($items)==$key+1) echo ' lastItem'; ?>">
					...
					<?php if($params->get('itemDateCreated')): ?>
						<div class="it-date">
							<span class="it-date-d"><?php echo JHTML::_('date', $item->created, JText::_('d')); ?></span>
							<span class="it-date-m"><?php echo JHTML::_('date', $item->created, JText::_('M')); ?></span>
						</div>
					<?php endif; ?>
					</div>
					<div class="it-right">
					...
					<?php if($params->get('itemTitle') || $params->get('itemIntroText')): ?>
						<div class="moduleItemIntrotext">
							<?php if($params->get('itemTitle')): ?>
								<a class="moduleItemTitle" href="<?php echo $item->link; ?>"><?php echo limit_text($item->title,4); ?></a>
							<?php endif; ?>

							<?php if($params->get('itemIntroText')): ?>
								<?php echo limit_text($item->introtext,13); ?>
							<?php endif; ?>
						</div>
					<?php endif; ?>

					<?php if($params->get('itemExtraFields') && count($item->extra_fields)): ?>
						<?php $fieldsevent = $item->extra_fields; ?>
						<?php if( (isset($fieldsevent[0]) && ($fieldsevent[0]->value !='')) || (isset($fieldsevent[1]) && ($fieldsevent[1]->value !=''))): ?>
							<div class="item-fields">
								<?php if( (isset($fieldsevent[0]) && ($fieldsevent[0]->value !=''))):?>
									<div class="fields-eventdate">
										<span><?php echo $fieldsevent[0]->value; ?></span>
									</div>
								<?php endif; ?>
								<?php if( (isset($fieldsevent[1]) && ($fieldsevent[1]->value !=''))):?>
									<div class="fields-eventaddress">
										<?php echo $fieldsevent[1]->value; ?>
									</div>
								<?php endif; ?>
							</div>
						<?php endif; ?>
