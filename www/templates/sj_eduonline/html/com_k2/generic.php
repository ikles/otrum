	...
	<div class="genericItemList">
		...
		<div class="genericItemView">
		  ...
		  <div class="genericItemBody">
			  ...
			  <!-- Item Image -->
			  <div class="genericItemImageBlock col-sm-3">
				  <span class="genericItemImage">
				    <a href="<?php echo $item->link; ?>" title="<?php if(!empty($item->image_caption)) echo K2HelperUtilities::cleanHtml($item->image_caption); else echo K2HelperUtilities::cleanHtml($item->title); ?>">
							<?php 
							//Create placeholder items images
							if(isset($item->imageGeneric)){
								$src = $item->imageGeneric;
							}else{
								$src = '';
							}
							$img_width = $this->params->get('itemImageGeneric');
							if (!empty( $src) && $src != '') {								
								$thumb_img = '<img src="'.$src.'" alt=""style="width:'.$img_width.'px; height:auto;" />';
							} else if ($is_placehold) {					
								$thumb_img = yt_placehold($placehold_size['k2_courses_itemrelated'],$item->title,$item->title);
							}	
							echo $thumb_img;
						?>
		  ...
		  <div class="genericItemExtraFields">
		  	<ul>
				<?php $item->extra_fields = $this->getItemExtraFields($item->extra_fields, $item);
