...
if (!class_exists('K2CategoriesBaseHelper')){
	/**
	 * BaseHelper for com_k2 only.
	 */
	abstract class K2CategoriesBaseHelper{
		/**
		...
		*/
		...
		public static function getK2CImages($item, $params, $prefix='imgcfg'){
			...
			if ( !isset(self::$image_cache[$hash][$item->id]) ){
				...
				$images_path = array();
				$priority = preg_split('/[\s|,|;]/', $params->get($prefix.'_order', 'external, k2_image, inline_description'), -1, PREG_SPLIT_NO_EMPTY);
				if ( count($priority) > 0 ){
					$priority = array_map('strtolower', $priority);
					$mark = array();
...
// 					}
					...
					foreach($mark as $type => $true){
						switch ($type){
							case 'k2_image':
								...
								$img_path = JPATH_SITE.'/media/k2/categories/'.$item->id;
								$img_hash = md5('Image'.$item->id);
								$img_suffix1 = '.jpg';
								$img_suffix2 = '.png';
								$img_suffix3 = '.gif';
								if (file_exists($img_path.$img_suffix1) ){
									$image = array(
											'src' => $img_path.$img_suffix1,
									);
									array_push($images_path, $image);
								}else if(file_exists($img_path.$img_suffix2)){
									$image = array(
											'src' => $img_path.$img_suffix2,
									);
									array_push($images_path, $image);
								}else if(file_exists($img_path.$img_suffix3)){
									$image = array(
											'src' => $img_path.$img_suffix3,
									);
									array_push($images_path, $image);
								}
			...
			if ($params->get('catfilter')){
				...
				if ( count($list)>0){
					foreach ($list as $i => $category){
						...
						$list[$i]->categoriesChild = array_slice($categoriesChild,0,$params->get('source_limit',6));
					}
				}
			} else {
	    ...
	    public static function getCategoryTree($categories, $maxlevel=0)
	    {
	        $mainframe = JFactory::getApplication();
	        $db = JFactory::getDBO();
	        $user = JFactory::getUser();
