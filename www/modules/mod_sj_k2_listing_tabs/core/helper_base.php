...
if (!class_exists('K2ListingTabsBaseHelper')) {
	/**
	 * BaseHelper for com_content only.
	 */
	abstract class K2ListingTabsBaseHelper
	{
		/**
			...
			if (!isset(self::$image_cache[$hash][$item->id])) {
				...
				if (count($images_path) == 0 && $params->get($prefix . '_placeholder', 1) == 1) {
                    if (strpos($params->get($prefix . '_placeholder_path', null), 'nophoto') === false) {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_k2_listing_tabs/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'respl-placeholder');
                    } else {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_k2_listing_tabs/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'placeholder');
                    }
                }
			...
			if (!isset(self::$image_cache[$hash][$item->id])) {
				...
				if (count($mark) > 0) {
					...
					foreach ($mark as $type => $true) {
						switch ($type) {
							case 'k2_image':
									if($item->k2_image != '' && $params->get('imgcfgcat_from_image_category') == 1){
										$src = 'media\k2\categories\\'.$item->k2_image;
										$src = str_replace('\\','/',$src);
										$image = array(
											'src' => $src
										);
