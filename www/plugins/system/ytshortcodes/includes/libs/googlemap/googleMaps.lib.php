...
";
			...
			$key_text = $this->key_text;
			$codes .= $this->setup_gmap($key_text);
...
";
	...
	function gmap_code() {
		$js = "\n<script type=\"text/javascript\">
<!--
function init_{$this->mod}_gmap(id, addr, centerLatitude, centerLongitude, startZoom, description, kml, control, maptype, show_marker, addoverview, addscale, addgoogle, streetview, type,key_text) {
    var map;
	if (addr!='') {
...
class Plugin_googleMaps extends Plugin_googleMaps_base {
	...
	function setup_gmap($key_text) {
		...
		{
			$output .= "\n"."<script type=\"text/javascript\" src=\"http://maps.googleapis.com/maps/api/js?sensor=false&key=".$key_text."&amp;language=".$this->lang.'"></script>';
